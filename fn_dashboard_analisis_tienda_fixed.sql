-- Sección corregida para Ventas Totales del período actual
SELECT 
    COALESCE(SUM(productos_por_operacion.total_importe), 0),
    COUNT(DISTINCT o.id)
INTO 
    v_ventas_totales, 
    v_total_ordenes
...
-- Sección corregida para Ventas Totales del período anterior
SELECT COALESCE(SUM(productos_por_operacion.total_importe), 0)
...
-- Sección corregida para Tendencias de Venta
WITH ventas_agrupadas AS (
    SELECT 
        ...
        END AS x_axis,
        COALESCE(SUM(productos_por_operacion.total_importe), 0) AS total_ventas
    FROM app_dat_operaciones o
    JOIN app_dat_operacion_venta ov ON o.id = ov.id_operacion
    JOIN app_dat_estado_operacion eo ON o.id = eo.id_operacion
    -- Subquery para agregar productos por operación
...
BEGIN
    ...
    -- Ventas Totales del período actual (CORREGIDO)
    SELECT 
        COALESCE(SUM(productos_por_operacion.total_importe), 0),
        COUNT(DISTINCT o.id)
    INTO 
    ...
    -- Ventas Totales del período anterior (CORREGIDO)
    SELECT COALESCE(SUM(productos_por_operacion.total_importe), 0)
    INTO v_ventas_totales_anterior
    FROM app_dat_operaciones o
    JOIN app_dat_operacion_venta ov ON o.id = ov.id_operacion
    ...
    -- Total de Gastos (Recepciones de productos con conversión USD)
    SELECT COALESCE(SUM(rp.cantidad * (rp.precio_unitario * tc.tasa)), 0)
    INTO v_total_gastos
    FROM app_dat_recepcion_productos rp
    INNER JOIN app_dat_operaciones o ON rp.id_operacion = o.id
    LEFT JOIN tasas_conversion tc ON tc.moneda_origen = 'USD'
    WHERE o.id_tienda = p_id_tienda 
      AND o.created_at BETWEEN v_fecha_inicio AND v_fecha_fin;

    -- Tendencias de Venta (CORREGIDO)
    WITH ventas_agrupadas AS (
        SELECT 
            CASE 
                WHEN p_periodo IN ('5 años', '3 años', '1 año') THEN 
