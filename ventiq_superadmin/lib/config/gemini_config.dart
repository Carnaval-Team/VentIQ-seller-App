...
class AssistantModelConfig {
  ...
  String _resolveParamName() {
    if (_normalizedParamKey == 'key') return 'key';
    return 'Authorization';
  }

  String _resolveParamValue() {
    if (_normalizedParamKey == 'bearer') return 'Bearer $apiKey';
    if (_normalizedParamKey == 'basic') return 'Basic $apiKey';
    return apiKey;
  }
...
class GeminiConfig {
  ...
  static Future<AssistantModelConfig> load({bool forceRefresh = false}) async {
    if (!forceRefresh && _cachedConfig != null && _lastFetchAt != null) {
      ...
      if (elapsed < _cacheTtl) return _cachedConfig!;
    }
