-- Versión optimizada de get_productos_completos_by_tienda
...
BEGIN
    ...
    productos_completos AS (
        ...
        GROUP BY p.id, p.sku, p.denominacion, p.nombre_comercial, p.descripcion, 
                 ...
                 pa.precio_venta_cup, sp.stock_total, sp.tiene_stock
...
-- CREAR ESTOS ÍNDICES EN LA BASE DE DATOS:

-- Índice compuesto para inventario (clave para rendimiento)
CREATE INDEX IF NOT EXISTS idx_inventario_productos_optimized 
ON app_dat_inventario_productos (id_producto, id_variante, id_opcion_variante, id_presentacion, id_ubicacion, id DESC)
WHERE cantidad_final > 0;

-- Índice para productos por tienda y categoría
CREATE INDEX IF NOT EXISTS idx_producto_tienda_categoria 
ON app_dat_producto (id_tienda, id_categoria, es_vendible) 
WHERE es_vendible = true;

-- Índice para precios vigentes
CREATE INDEX IF NOT EXISTS idx_precio_venta_vigente 
ON app_dat_precio_venta (id_producto, fecha_desde DESC, fecha_hasta) 
WHERE (id_variante IS NULL OR id_variante = 0);

-- Índices para relaciones frecuentes
CREATE INDEX IF NOT EXISTS idx_productos_subcategorias_producto 
ON app_dat_productos_subcategorias (id_producto);

CREATE INDEX IF NOT EXISTS idx_producto_presentacion_producto 
ON app_dat_producto_presentacion (id_producto);

CREATE INDEX IF NOT EXISTS idx_producto_multimedias_producto 
ON app_dat_producto_multimedias (id_producto);

CREATE INDEX IF NOT EXISTS idx_producto_etiquetas_producto 
ON app_dat_producto_etiquetas (id_producto);
