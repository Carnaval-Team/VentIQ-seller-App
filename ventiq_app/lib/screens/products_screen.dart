...
class _SubcategorySectionState extends State<_SubcategorySection> {
  ...
  double _scrollStep(BuildContext context) {
    return _columnWidth(context) + _columnGap(context);
  }
  ...
  Widget _buildProductsList(BuildContext context) {
    ...
    final listView = ListView.separated(
      ...
      itemCount: (products.length / itemsPerColumn).ceil(),
      separatorBuilder: (context, index) => SizedBox(width: columnGap),
      itemBuilder: (context, columnIndex) {
        // Calcular productos para esta columna
        final startIndex = columnIndex * itemsPerColumn;
        final endIndex = (startIndex + itemsPerColumn).clamp(0, products.length);
