-- Script para actualizar la función fn_inventario_resumen_por_usuario
-- Agrega filtros por tienda y almacén, y corrige el cálculo de stock disponible

-- Primero eliminar la función existente si existe
DROP FUNCTION IF EXISTS fn_inventario_resumen_por_usuario();

-- Crear la nueva función mejorada
CREATE OR REPLACE FUNCTION fn_inventario_resumen_por_usuario(
...
GRANT EXECUTE ON FUNCTION fn_inventario_resumen_por_usuario TO authenticated;

-- Comentario sobre la función actualizada
COMMENT ON FUNCTION fn_inventario_resumen_por_usuario IS 
'Función mejorada para obtener resumen de inventario por usuario con filtros por tienda y almacén. 
Incluye cálculo correcto de stock disponible descontando stock reservado, paginación y búsqueda por texto.
Parámetros:
- p_id_tienda: Filtrar por tienda específica (opcional)
- p_id_almacen: Filtrar por almacén específico (opcional, requiere p_id_tienda)
- p_busqueda: Buscar en denominación, SKU, código de barras y variantes (opcional)
- p_mostrar_sin_stock: Incluir productos sin stock (default: true)
- p_limite: Número de registros por página (default: 50)
- p_pagina: Número de página (default: 1)

Retorna:
- prod_id, prod_nombre: Información del producto
- variante_id, variante_valor: Información de variante
- opcion_variante_id, opcion_variante_valor: Información de opción de variante
- cant_unidades_base: Cantidad en unidades base
- cant_almacen_total: Cantidad total en almacén
- stock_disponible: Stock disponible (descontando reservado)
- stock_reservado: Stock reservado en operaciones pendientes
- zonas_count: Número de zonas donde está el producto
- presentaciones_count: Número de presentaciones del producto
- total_count: Total de registros (para paginación)';

-- Ejemplos de uso:
-- SELECT * FROM fn_inventario_resumen_por_usuario(); -- Todos los productos
-- SELECT * FROM fn_inventario_resumen_por_usuario(p_id_tienda := 1); -- Solo tienda 1
-- SELECT * FROM fn_inventario_resumen_por_usuario(p_id_tienda := 1, p_id_almacen := 5); -- Solo almacén 5 de tienda 1
-- SELECT * FROM fn_inventario_resumen_por_usuario(p_busqueda := 'aceite', p_limite := 20, p_pagina := 1); -- Buscar 'aceite' con paginación
