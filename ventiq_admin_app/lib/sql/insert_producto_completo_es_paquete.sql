-- Modificación del RPC insertar_producto_completo
-- Agrega soporte para el campo es_paquete en app_dat_producto
-- Solo se muestran los cambios relevantes: la columna se añade al INSERT principal.
...
BEGIN
    ...
    -- ✅ INSERTAR PRODUCTO PRINCIPAL (AGREGADO: es_paquete)
    INSERT INTO app_dat_producto (
        ...
        CASE
            ...
            ELSE NULL
        END,
        COALESCE((producto_data->>'es_paquete')::BOOLEAN, FALSE)
    ...
    -- Loguear es_paquete
    IF COALESCE((producto_data->>'es_paquete')::BOOLEAN, FALSE) THEN
        RAISE NOTICE '   📦 Producto marcado como paquete';
        inserted_data := jsonb_set(inserted_data, '{es_paquete}', 'true'::jsonb);
    END IF;
