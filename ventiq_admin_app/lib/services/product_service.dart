...
class ProductService {
  ...
  static Future<Map<String, dynamic>?> findProductByNameAndStore({
    ...
    try {
      print(
        '🔍 Buscando producto existente: "$denominacion" en tienda: $idTienda',
      );
      ...
      for (final producto in response) {
        final denomProducto =
            (producto['denominacion'] as String?)?.trim().toLowerCase() ?? '';
      ...
      print(
        '✅ Producto existente encontrado: ID=${productoEncontrado['id']}, SKU=${productoEncontrado['sku']}',
      );
  ...
  static Future<Map<String, dynamic>> createSubcategoria({
    ...
    try {
      print(
        '🏗️ Creando subcategoría: $denominacion para categoría: $idCategoria',
      );
      ...
      final response = await _supabase
          ...
          .insert({
            'idcategoria':
                idCategoria, // Nombre correcto del campo según schema

            'denominacion': denominacion,

            'sku_codigo': _generateSkuFromName(
              denominacion,
            ), // Campo obligatorio
          })
          .select('id')
    ...
    if (sku.length < 3) {
      sku = cleanName
          ...
          .substring(
            ...
            cleanName.replaceAll(' ', '').length > 6
                ? 6
                : cleanName.replaceAll(' ', '').length,
          );
    }
  ...
  static Product _convertToProduct(Map<String, dynamic> json) {
    try {
      ...
      return Product(
        ...
        multimedias: (json['multimedias'] as List<dynamic>? ?? [])
        ...
        inventario: (json['inventario'] as List<dynamic>? ?? [])
            .cast<Map<String, dynamic>>(),
        ...
        nombreProveedor:
            json['nombre_proveedor'] ??
            (json['app_dat_proveedor']
  ...
  static Future<List<Map<String, dynamic>>> getProductReceptionCostAudit(
    String productId,
  ) async {
    final id = int.tryParse(productId);
    if (id == null) return [];

    final receptions = List<Map<String, dynamic>>.from(
      await _supabase
    ...
    final operationIds = receptions
        .map((row) => (row['id_operacion'] as num?)?.toInt())
        .whereType<int>()
        .toSet()
        .toList();
    if (operationIds.isEmpty) return receptions;

    final operations = List<Map<String, dynamic>>.from(
      await _supabase
          .from('app_dat_operaciones')
          .select('*')
          .inFilter('id', operationIds),
    );
    final receptionHeaders = List<Map<String, dynamic>>.from(
      await _supabase
    ...
    final operationById = {
      for (final operation in operations)
        operation['id']?.toString(): operation,
    };
    final headerById = {
      for (final header in receptionHeaders)
        header['id_operacion']?.toString(): header,
    };

    return receptions
        .map((row) {
          final operationId = row['id_operacion']?.toString();
          return {
            ...row,
            'operacion': operationById[operationId],
            'recepcion': headerById[operationId],
          };
        })
        .where((row) {
          final header = row['recepcion'];
          if (header is! Map) return true;
          final motivo = header['motivo'];
          final motivoId = motivo is num
              ? motivo.toInt()
              : int.tryParse(motivo?.toString() ?? '');
          return motivoId != 2;
        })
        .toList();
  }
  ...
  static Future<List<Map<String, dynamic>>> getProductPromotionalPrices(
    ...
    try {
      ...
      final promociones = data.map<Map<String, dynamic>>((promo) {
        // Convertir valores con logging detallado

        final nombre = promo['nombre']?.toString() ?? 'Promoción sin nombre';
  ...
  static Future<List<Map<String, dynamic>>> getProductStockHistory(
    ...
    try {
      ...
      final ultimoStock = stockHistory.isNotEmpty
          ? stockHistory.last['cantidad']
          : 0.0;
  ...
  static double _resolveSignedMovementQty(Map<dynamic, dynamic> operation) {
    ...
    if (qty.abs() <= epsilon) {
      final ini =
          (operation['stock_inicial'] as num?)?.toDouble() ??
          (operation['cantidad_inicial'] as num?)?.toDouble();
      final fin =
          (operation['stock_final'] as num?)?.toDouble() ??
  ...
  static Future<bool> updateProductSupplier(
    ...
    try {
      ...
      final response = await _supabase.rpc(
        ...
        params: {'p_id_producto': productId, 'p_id_proveedor': supplierId},
      );
  ...
  static Future<int?> _getStoreId([int? providedStoreId]) async {
    try {
      ...
      if (!_storeSelectorService!.isInitialized) {
        ...
        final storeIdAfterInit = await _storeSelectorService!
            .getSelectedStoreId();
  ...
  static Future<Map<String, dynamic>> updateReceptionOperation({
    ...
    try {
      ...
      if (fechaFactura != null)
        params['p_fecha_factura'] = fechaFactura.toIso8601String().split(
          'T',
        )[0];
  ...
  static Future<Product?> getProductoCompletoById(int productId) async {
    try {
      ...
      final presentaciones = presentacionesResponse.map<Map<String, dynamic>>((
        item,
      ) {
        final itemMap = item as Map<String, dynamic>;

        final nomPres =
            itemMap['app_nom_presentacion'] as Map<String, dynamic>?;
      ...
      final etiquetas = etiquetasResponse
          .map<String>(
            (item) => (item as Map<String, dynamic>)['etiqueta'] as String,
          )
          .toList();
      ...
      final subcategoriasMapped = subcategoriasResponse
          .map<Map<String, dynamic>>((item) {
            ...
            final subcat =
                itemMap['app_dat_subcategorias'] as Map<String, dynamic>?;
      ...
      try {
        ...
        if (precioVentaResponse.isNotEmpty) {
          precioVenta =
              (precioVentaResponse.first['precio_venta_cup'] as num?)
                  ?.toDouble() ??
              0.0;

          precioVentaUsd =
              (precioVentaResponse.first['precio_venta_usd'] as num?)
                  ?.toDouble();
      ...
      return Product(
        ...
        updatedAt:
            DateTime.now(), // Usar fecha actual ya que no existe updated_at en BD
        ...
        diasAlertCaducidad:
            (productData['dias_alert_caducidad'] as num?)?.toInt() ?? 0,
  ...
  static Future<List<Map<String, dynamic>>> getProductsUsingThisIngredient(
    ...
    try {
      ...
      final response = await _supabase.rpc(
        ...
        params: {'p_id_producto_ingrediente': int.parse(productId)},
      );
      ...
      print(
        '📊 Total productos encontrados que usan este ingrediente: ${productosData.length}',
      );
  ...
  static Future<String?> _uploadProductImage(
    ...
    try {
      ...
      final uniqueFileName =
          'product_${DateTime.now().millisecondsSinceEpoch}_$fileName';
  ...
  static Future<bool> removeProductImage({required String productId}) async {
    try {
  ...
  static Future<bool> updateProductShortNameByDenomination(
    ...
    try {
      debugPrint(
        '🔍 Actualizando denominación corta por denominación: $denominacion',
      );
      ...
      if (success) {
        ...
        debugPrint(
          '📝 Denominación anterior: ${response['previous_short_name']}',
        );
  ...
  static Future<Map<String, dynamic>> updateMultipleProductShortNames(
    ...
    try {
      debugPrint(
        '🔄 Iniciando actualización masiva de ${actualizaciones.length} productos',
      );
      ...
      final jsonActualizaciones = actualizaciones
          .map(
            (item) => {
              'denominacion': item['denominacion'],
      ...
      debugPrint(
        '🔄 Llamando a función RPC: fn_actualizar_denominacion_corta_masivo',
      );

      debugPrint(
        '📊 Parámetros: p_id_tienda=$idTienda, p_actualizaciones=$jsonActualizaciones',
      );
  ...
  static Future<Map<String, dynamic>> initializePresentationAveragePrices({
    ...
    try {
      ...
      for (final pres in presentaciones) {
        ...
        final cantidadPresentacion =
            (pres['cantidad'] as num?)?.toDouble() ?? 1.0;

        debugPrint(
          '🔄 Procesando presentación ID: $idPresentacion (cantidad: $cantidadPresentacion)',
        );
        ...
        final operacionesPorTienda = (operaciones as List<dynamic>).where((op) {
          final opData = op as Map<String, dynamic>;

          final operacionData =
              opData['app_dat_operaciones'] as Map<String, dynamic>?;
        ...
        if (operacionesPorTienda.isNotEmpty) {
          ...
          debugPrint(
            '  📊 Basado en ${operacionesPorTienda.length} operaciones de recepción',
          );
          ...
          debugPrint(
            '  ⚠️ No hay operaciones de recepción para este producto en esta tienda',
          );
        }
      }

      debugPrint(
        '✅ Inicialización completada: $updated presentaciones actualizadas',
      );
  ...
  static Future<bool> updatePresentationAveragePrice({
    ...
    try {
      debugPrint(
        '💰 Actualizando precio promedio de presentación: $presentationId',
      );
  ...
  static Future<Map<String, dynamic>> calcularCostoProductoElaborado(
    int productId,
  ) async {
    try {
      print(
        '🧮 Iniciando cálculo de costo para producto elaborado ID: $productId',
      );
      ...
      for (final ingrediente in ingredientes) {
        ...
        final cantidadNecesaria =
            (ingrediente['cantidad_necesaria'] as num?)?.toDouble() ?? 0.0;
        final unidadMedida = ingrediente['unidad_medida'] as String? ?? '';

        print(
          '🔍 Calculando costo para ingrediente ID: $idIngrediente, Cantidad: $cantidadNecesaria $unidadMedida',
        );

        // Obtener costo unitario del ingrediente
        final costoUnitario = await _calcularCostoUnitarioIngrediente(
          idIngrediente,
        );
        ...
        print(
          '✅ Ingrediente: ${productoIngrediente['denominacion']} - Costo: $costoIngrediente',
        );
      }

      print('🧮 Costo total calculado para producto $productId: $costoTotal');

      return {'costo_total': costoTotal, 'desglose': desglose, 'error': null};
    } catch (e) {
  ...
  static Future<bool> esProductoElaborado(int productId) async {
    try {
      ...
      print(
        '📋 Producto $productId elaborado: $isElaborado (response: $response)',
      );
      return isElaborado;
    } catch (e) {
  ...
  /// Obtiene los ingredientes de un producto elaborado
  static Future<List<Map<String, dynamic>>> obtenerIngredientesProducto(
    int productId,
  ) async {
    try {
      print('🍽️ Obteniendo ingredientes para producto elaborado: $productId');
