...
import 'estados_deposito_screen.dart';
import 'bancos_screen.dart';

class DepositosBancariosScreen extends StatefulWidget {
  const DepositosBancariosScreen({super.key});

  @override
  State<DepositosBancariosScreen> createState() =>
      _DepositosBancariosScreenState();
}

class _DepositosBancariosScreenState extends State<DepositosBancariosScreen>
    with SingleTickerProviderStateMixin {
  final DepositosBancariosService _service = DepositosBancariosService();
  late TabController _tabController;

  List<BancoDeposito> _bancos = [];
  BancoDeposito? _bancoSeleccionado;
  // currency comes from selected bank
  bool _isLoadingBancos = true;

  double _saldoDisponible = 0.0;
  List<RecargaSaldoDeposito> _recargas = [];
  ...
  int? get _idBanco => _bancoSeleccionado?.id;

  String get _monedaSimbolo => _bancoSeleccionado?.simboloMoneda ?? '\$';

  String get _monedaCodigo => _bancoSeleccionado?.codigoMoneda ?? 'USD';
  ...
  void initState() {
    ...
    _tabController.addListener(() {
      if (!_tabController.indexIsChanging && _idBanco != null) {
  ...
  Future<void> _loadBancos() async {
    ...
    try {
      final proveedores = await _service.getBancos(soloActivos: true);
      setState(() {
        _bancos = proveedores;
        _isLoadingBancos = false;
      });
    } catch (e) {
      setState(() => _isLoadingBancos = false);
      _showError('Error cargando bancos: $e');
    }
  }

  Future<void> _onBancoChanged(BancoDeposito? banco) async {
    setState(() {
  ...
  Future<void> _loadAllData() async {
    if (_idBanco == null) return;
    await Future.wait([_loadSaldoData(), _loadDepositosData()]);
  }
  ...
  Future<void> _loadSaldoData() async {
    ...
    try {
      ...
      setState(() {
        ...
        _recargas = results[1] as List<RecargaSaldoDeposito>;
        _historialSaldo = results[2] as List<HistorialSaldoDeposito>;
  ...
  Future<void> _loadDepositosData() async {
    final id = _idBanco;
    if (id == null) return;
    setState(() => _isLoadingDepositos = true);
    try {
      final results = await Future.wait([
  ...
  Future<void> _confirmCancelarPago(HistorialSaldoDeposito h) async {
  ...
  Widget? _buildCancelarPagoButton(HistorialSaldoDeposito h) {
  ...
  void _showRecargaDialog() {
    if (_idBanco == null) {
      _showError('Selecciona un banco primero');
      return;
    }
  ...
  void _showGestionFotosDialog(DepositoBancario factura) {
    showDialog(
      context: context,
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) {
                ...
                children: [
                              ...
                              Positioned(
                                ...
                                child: GestureDetector(
                                  onTap: () async {
                                    ...
                                    if (confirm == true) {
                                      try {
                                        await _service.eliminarFotoDeposito(
                                          foto.id!,
                                        );
                  ...
                  ElevatedButton.icon(
                    ...
                    onPressed: () async {
                      ...
                      Navigator.pop(ctx);
                      try {
                        await _service.agregarFotoDeposito(
                          idDeposito: factura.id!,
                          bytes: picked.bytes,
            ...
            ElevatedButton(
              onPressed: () {
                ...
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => const EstadosDepositoScreen(),
                  ),
                ).then((_) => _loadDepositosData());
              },
  ...
  void _showEditarDepositoDialog(DepositoBancario factura) {
    final numCtrl = TextEditingController(text: factura.numeroDeposito);
    final valorCtrl = TextEditingController(
        ...
        actions: [
          ...
          ElevatedButton.icon(
            ...
            onPressed: () async {
              ...
              Navigator.pop(ctx);
              try {
                await _service.actualizarDetallesDeposito(
                  idBanco: _idBanco!,
                  idDeposito: factura.id!,
                  numeroDepositoAnterior: factura.numeroDeposito,
  ...
  void _showCambioEstadoDialog(DepositoBancario factura) {
            ...
            ElevatedButton(
              onPressed: selectedEstadoId == factura.idEstado
                  ...
                  : () async {
                      Navigator.pop(ctx);
                      try {
                        await _service.cambiarEstadoDeposito(
                          idDeposito: factura.id!,
  ...
  void _showHistorialEstadosDialog(DepositoBancario factura) async {
    showDialog(
      context: context,
      builder: (ctx) => FutureBuilder<List<HistorialEstadoDeposito>>(
  ...
  Future<void> _openBancosScreen() async {
    ...
    await _loadBancos();
    if (_idBanco != null) {
      final bancos = await _service.getBancos();
      final match = bancos.where((b) => b.id == _idBanco).toList();
      if (match.isNotEmpty) {
        setState(() => _bancoSeleccionado = match.first);
      }
      await _loadAllData();
    }
  }
    ...
    return Scaffold(
          ...
          PopupMenuButton<String>(
            ...
            onSelected: (value) async {
              if (value == 'estados') {
                ...
                await Navigator.pushNamed(context, '/depositos-monedas');
                await _loadBancos();
                if (_idBanco != null) await _loadAllData();
              }
            },
            itemBuilder: (ctx) => [
              const PopupMenuItem(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: _idBanco == null
                ? _buildSeleccionarBancoEmpty()
                : TabBarView(
                    controller: _tabController,
                    children: [_buildSaldoTab(), _buildDepositosTab()],
                  ),
          ),
        ],
      ),
      floatingActionButton: _idBanco == null ? null : _buildFAB(),
    );
  }
          ...
          children: [
            DropdownButtonFormField<int>(
              ...
              items: _bancos
                  ...
                  .toList(),
              onChanged: _isLoadingBancos
                  ? null
                  : (id) {
                      BancoDeposito? proveedor;
                      if (id != null) {
                        for (final p in _bancos) {
                          if (p.id == id) {
                            proveedor = p;
                            break;
                          }
              ...
              Row(
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(
                      fontSize: 13,
                      color: _bancoSeleccionado != null
                          ? Colors.grey[700]
                          : Colors.orange[800],
                      fontWeight: _bancoSeleccionado != null
                          ? FontWeight.normal
                          : FontWeight.w600,
                    ),
                  ),
                  const Spacer(),
                  TextButton(
                    onPressed: _openBancosScreen,
                    child: Text(
                      _bancoSeleccionado != null ? 'Cambiar' : 'Configurar',
                      style: const TextStyle(fontSize: 12),
                    ),
                  ),
                ],
  ...
  Widget _buildSeleccionarBancoEmpty() {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        if (_tabController.index == 0) {
          ...
          return FloatingActionButton.extended(
            onPressed: _showNuevoDepositoDialog,
            backgroundColor: AppColors.primary,
            foregroundColor: Colors.white,
            icon: const Icon(Icons.add),
  ...
  Future<void> _exportarReportePdf() async {
    ...
    final proveedorNombre = _bancoSeleccionado?.denominacion ?? 'Proveedor';
    ...
    await Printing.layoutPdf(
      ...
      name:
          'reporte_pago_bancos_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf',
    );
  }

  List<HistorialSaldoDeposito> _historialFiltrado() {
  ...
  Widget _buildDepositosTab() {
    ...
    return RefreshIndicator(
      onRefresh: _loadDepositosData,
      color: AppColors.primary,
      child: _depositos.isEmpty
          ? ListView(
              physics: const AlwaysScrollableScrollPhysics(),
              children: [
                const SizedBox(height: 80),
          ...
          : ListView.separated(
              ...
              separatorBuilder: (_, __) => const SizedBox(height: 8),
              itemBuilder: (ctx, i) => _buildDepositoCard(_depositos[i]),
            ),
    );
  }
