...
class InventoryOperationsWebScreen extends StatefulWidget {
  const InventoryOperationsWebScreen({super.key});

  @override
  State<InventoryOperationsWebScreen> createState() =>
      _InventoryOperationsWebScreenState();
}

class _InventoryOperationsWebScreenState extends State<InventoryOperationsWebScreen> {
  ...
  void initState() {
    super.initState();
    print('🚀 InventoryOperationsWebScreen inicializado');
                      ...
                      children: [
                        ...
                        Wrap(
                          ...
                          children: [
                            ...
                            _buildQuickDateButton('Ayer', () {
                              _setQuickDateRange(1);
                              setLocalState(() {});
                            }),
                            _buildQuickDateButton('Últimos 7 días', () {
                              _setQuickDateRange(7);
                              setLocalState(() {});
                            }),
                            _buildQuickDateButton('Últimos 15 días', () {
                              _setQuickDateRange(15);
                              setLocalState(() {});
                            }),
                            _buildQuickDateButton('Últimos 30 días', () {
                            ...
                            _buildQuickDateButton('Mes anterior', () {
                              _setPreviousMonth();
                              setLocalState(() {});
                            }),
                          ],
                        ...
                        SizedBox(
                          ...
                          child: OutlinedButton.icon(
                            onPressed: () async {
                              await _showNativeDateRangePicker();
                              setLocalState(() {});
                            },
                            icon: const Icon(
  ...
  Widget _buildFilters() {
    final hasFilters = _fechaDesde != null ||
        _fechaHasta != null ||
        _tipoOperacionId != null;

    final dateLabel = (_fechaDesde != null && _fechaHasta != null)
        ? '${_formatDate(_fechaDesde!)} - ${_formatDate(_fechaHasta!)}'
        : 'Cualquier fecha';

    final typeLabel = _tipoOperacionId == null
        ? 'Todos los tipos'
        : _operationTypeName(_tipoOperacionId!);
    ...
    return Padding(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final searchField = _buildOpsSearchField();
          final dateSelect = Builder(
            builder: (ctx) => _buildOpsFilterSelect(
              icon: Icons.date_range_rounded,
              label: 'Fechas',
              value: dateLabel,
              color: const Color(0xFF06B6D4),
              active: _fechaDesde != null && _fechaHasta != null,
              onTap: _showDateRangeDialog,
            ),
          );
          final typeSelect = Builder(
            builder: (ctx) => _buildOpsFilterSelect(
              icon: Icons.filter_list_rounded,
              label: 'Tipo',
              value: typeLabel,
              color: const Color(0xFF8B5CF6),
              active: _tipoOperacionId != null,
              onTap: () => _showOperationTypeMenu(ctx),
            ),
          );
          final clearBtn = hasFilters ? _buildClearFiltersButton() : null;
          ...
          if (isWide) {
            return Row(
              children: [
                ...
                Expanded(flex: 3, child: dateSelect),
                const SizedBox(width: 8),
                Expanded(flex: 3, child: typeSelect),
                if (clearBtn != null) ...[
                  const SizedBox(width: 8),
                  clearBtn,
                ],
  ...
  String _operationTypeName(int id) {
    switch (id) {
      case 1:
        return 'Recepción';
      case 2:
  ...
  Future<void> _showOperationTypeMenu(BuildContext anchorContext) async {
    ...
    final options = <(int?, String, IconData, Color)>[
      (null, 'Todos los tipos', Icons.all_inclusive_rounded, AppColors.textSecondary),
      (1, 'Recepción de Productos', Icons.input_rounded, const Color(0xFF10B981)),
      (2, 'Extracción de Productos', Icons.output_rounded, const Color(0xFFEF4444)),
      (3, 'Ajuste de Inventario', Icons.tune_rounded, const Color(0xFFFF6B35)),
      (4, 'Venta', Icons.shopping_cart_rounded, const Color(0xFF4A90E2)),
      (5, 'Apertura de Caja', Icons.point_of_sale_rounded, const Color(0xFF10B981)),
      (6, 'Cierre de Caja', Icons.point_of_sale_rounded, const Color(0xFFFF8C42)),
      (7, 'Transferencia', Icons.swap_horiz_rounded, const Color(0xFF8B5CF6)),
    ];
                  ...
                  Expanded(
                    child: Text(
                      ...
                      style: TextStyle(
                        fontSize: 13,
                        color: AppColors.textPrimary,
                        fontWeight: _tipoOperacionId == o.$1
                            ? FontWeight.w700
                            : FontWeight.w500,
                      ),
                      maxLines: 1,
    ...
    if (!mounted) return;
    setState(() => _tipoOperacionId = selected);
    _currentPage = 1;
    _loadOperations();
  }
