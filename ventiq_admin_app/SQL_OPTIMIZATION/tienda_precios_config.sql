  ...
  begin
    ...
    select p.id::bigint,
          p.denominacion::text,
          p.sku::text,
          p.id_vendedor_app::bigint,
          pv.precio_venta_cup::numeric,
          pv.precio_venta_usd::numeric,
          pv.created_at::timestamptz
    from app_dat_producto p
    left join lateral (
      select apv.precio_venta_cup,
            apv.precio_venta_usd,
            apv.created_at
      from app_dat_precio_venta apv
      where apv.id_producto = p.id
