...
1. **`migraciones/`** completo (01 → 02 → 03).
2. **`rpc clientes/00_indices_sala_espera.sql`** (índices de apoyo).
3. El resto de **`rpc clientes/`** (cualquier orden; el 08 depende del 07).
4. **`rpc admin/00_helper_entidades_usuario.sql`** primero, luego el resto de `rpc admin/`.
