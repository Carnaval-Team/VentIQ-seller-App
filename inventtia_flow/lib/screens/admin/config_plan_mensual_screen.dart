...
/// local_servicio + generación de los planes de un mes en lote.
///
/// Dos modos, según el servicio use o no recursos:
///  • SIN recursos: una capacidad por defecto + override por día (cupo único).
///  • CON recursos: una capacidad por recurso (por defecto + override por día).
///    Todos los tramos del recurso reciben esa capacidad ese día.
...
class _ConfigPlanMensualScreenState extends State<ConfigPlanMensualScreen> {
  ...
  final Map<int, _RecursoCtrls> _recCtrls = {};

  bool get _porRecurso => _recursos.isNotEmpty;
  ...
  void dispose() {
    ...
    for (final rc in _recCtrls.values) {
      rc.dispose();
    }
    super.dispose();
  }
  ...
  Future<void> _cargar() async {
    ...
    try {
      // 1) ¿El servicio usa recursos? Determina el modo de la pantalla.
      final recursos = await RecursoService.listar(
        uuidUsuario: _uuid,
        idLocalServicio: widget.localServicio.id,
      );
      final activos = recursos.where((r) => r.activo).toList();

      // 2) Config guardada (si la hay).
      ...
      if (_porRecurso) {
        for (final r in activos) {
          final rc = _RecursoCtrls();
          final saved = cfg?.porRecurso[r.id];
          rc.defaultCtrl.text = (saved?.porDefecto ?? 1).toString();
          for (var d = 1; d <= 7; d++) {
            rc.diaCtrls[d]!.text = (saved != null && saved.porDia.containsKey(d))
                ? saved.porDia[d].toString()
                : '';
          }
          _recCtrls[r.id] = rc;
        }
  ...
  PlanConfig _buildConfig() {
    if (_porRecurso) {
      ...
      for (final entry in _recCtrls.entries) {
        final rc = entry.value;
        final def = int.tryParse(rc.defaultCtrl.text.trim()) ?? 0;
        final porDia = <int, int>{};
        for (var d = 1; d <= 7; d++) {
          final txt = rc.diaCtrls[d]!.text.trim();
          if (txt.isEmpty) continue; // vacío = usar default
          final v = int.tryParse(txt);
          if (v != null) porDia[d] = v;
        }
        porRecurso[entry.key] =
            RecursoPlanConfig(porDefecto: def, porDia: porDia);
      }
      return PlanConfig(
        idLocalServicio: widget.localServicio.id,
        ...
        showDialog(
          ...
          builder: (_) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                _ResumenRow(
                    label: _porRecurso ? 'Tramos-día creados' : 'Creados',
                    valor: res.creados,
    ...
    return Scaffold(
      ...
      body: _loading
          ...
          : ListView(
              ...
              children: [
                if (_porRecurso) ..._buildRecursoSection() else ..._buildSimpleSection(),
  ...
  // ── Sección modo SIN recursos ──
  List<Widget> _buildSimpleSection() => [
        ...
        TextField(
          ...
          onChanged: (_) => setState(() {}), // refresca los hints de los días
        ),
        const SizedBox(height: 16),
        Card(
          elevation: 0,
  ...
  // ── Sección modo CON recursos (una tarjeta por recurso) ──
  List<Widget> _buildRecursoSection() => [
        _sectionTitle('Capacidad por recurso y día'),
        const SizedBox(height: 4),
        const Text(
          'Define la capacidad de cada recurso: un valor por defecto y, '
          'opcionalmente, uno distinto por día. Deja un día vacío para usar el '
          'valor por defecto, o pon 0 para cerrar ese recurso ese día.',
          style: TextStyle(fontSize: 12.5, color: AppTheme.textSecondary),
        ),
        const SizedBox(height: 14),
        for (final r in _recursos) ...[
          _RecursoConfigCard(
            recurso: r,
            ctrls: _recCtrls[r.id]!,
            nombresDia: _nombresDia,
            onDefaultChanged: () => setState(() {}),
          ),
...
/// Controladores de la config de un recurso (default + 7 días).
class _RecursoCtrls {
  final TextEditingController defaultCtrl = TextEditingController(text: '1');
  final Map<int, TextEditingController> diaCtrls = {
    for (var d = 1; d <= 7; d++) d: TextEditingController(),
  };

  void dispose() {
    defaultCtrl.dispose();
    for (final c in diaCtrls.values) {
      c.dispose();
    }
  }
}

/// Tarjeta de configuración de un recurso: capacidad por defecto + por día.
class _RecursoConfigCard extends StatelessWidget {
  final Recurso recurso;
  final _RecursoCtrls ctrls;
  final List<String> nombresDia;
  final VoidCallback onDefaultChanged;

  const _RecursoConfigCard({
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 6),
            for (var d = 1; d <= 7; d++) ...[
              if (d > 1) Divider(height: 1, color: Colors.grey.shade100),
              _DiaRow(
                nombre: nombresDia[d - 1],
                controller: ctrls.diaCtrls[d]!,
                hintDefault: ctrls.defaultCtrl.text.trim(),
              ),
            ],
          ],
        ),
