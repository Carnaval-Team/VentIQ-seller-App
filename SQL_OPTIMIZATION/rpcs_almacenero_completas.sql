-- =====================================================
-- FUNCIONES RPC COMPLETAMENTE NUEVAS PARA ALMACENERO
-- NO actualiza funciones existentes, crea nuevas
-- =====================================================

-- =====================================================
-- RPC: fn_crear_trabajador_almacenero
-- Descripción: Crea trabajador con rol almacenero en una operación
-- =====================================================

CREATE OR REPLACE FUNCTION public.fn_crear_trabajador_almacenero(
    p_id_tienda INTEGER,
    p_nombres VARCHAR,
    p_apellidos VARCHAR,
    p_usuario_uuid UUID,
...
BEGIN
    ...
    INSERT INTO public.app_dat_trabajadores (
        id_tienda, nombres, apellidos, usuario_uuid, salario_horas, estado
    )
    VALUES (p_id_tienda, p_nombres, p_apellidos, p_usuario_uuid, p_salario_horas, 1)
    RETURNING id INTO v_trabajador_id;

    -- Crear almacenero
    INSERT INTO public.app_dat_almacenero (
        id_trabajador, id_almacen, id_tienda, estado
    )
    VALUES (v_trabajador_id, p_almacen_id, p_id_tienda, 1)
...
-- =====================================================
-- RPC: fn_asignar_rol_almacenero
-- Descripción: Asigna rol almacenero a trabajador existente
-- =====================================================

CREATE OR REPLACE FUNCTION public.fn_asignar_rol_almacenero(
...
BEGIN
    ...
    IF v_id_tienda IS NULL THEN
        RETURN json_build_object('success', false, 'message', 'Trabajador no encontrado');
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RETURN json_build_object('success', false, 'message', 'Almacén inválido');
    END IF;

    -- Verificar si ya es almacenero
    IF EXISTS (
        SELECT 1 FROM public.app_dat_almacenero 
        WHERE id_trabajador = p_trabajador_id AND id_tienda = v_id_tienda AND estado = 1
    ) THEN
        RETURN json_build_object('success', false, 'message', 'Ya es almacenero');
    END IF;

    -- Crear o reactivar
    INSERT INTO public.app_dat_almacenero (id_trabajador, id_almacen, id_tienda, estado)
    VALUES (p_trabajador_id, p_almacen_id, v_id_tienda, 1)
    ON CONFLICT (id_trabajador, id_tienda) 
    DO UPDATE SET id_almacen = p_almacen_id, estado = 1, updated_at = NOW()
    RETURNING id INTO v_almacenero_id;

    RETURN json_build_object('success', true, 'message', 'Rol asignado', 'almacenero_id', v_almacenero_id);
...
-- =====================================================
-- RPC: fn_cambiar_almacen_almacenero
-- Descripción: Cambia el almacén asignado a un almacenero
-- =====================================================

CREATE OR REPLACE FUNCTION public.fn_cambiar_almacen_almacenero(
    p_trabajador_id INTEGER,
    p_almacen_id INTEGER
)
RETURNS JSON
...
BEGIN
    ...
    SET id_almacen = p_almacen_id, updated_at = NOW()
    WHERE id_trabajador = p_trabajador_id AND id_tienda = v_id_tienda AND estado = 1;

    GET DIAGNOSTICS v_rows_affected = ROW_COUNT;

    IF v_rows_affected = 0 THEN
        RETURN json_build_object('success', false, 'message', 'No es almacenero');
    END IF;

    RETURN json_build_object('success', true, 'message', 'Almacén actualizado');
...
-- =====================================================
-- RPC: fn_listar_trabajadores_completo
-- Descripción: Lista trabajadores con TODOS sus roles (nueva función)
-- =====================================================

CREATE OR REPLACE FUNCTION public.fn_listar_trabajadores_completo(
    p_id_tienda INTEGER,
...
BEGIN
    ...
    FROM (
        ...
        WHERE t.id_tienda = p_id_tienda AND t.estado = 1
...
-- =====================================================
-- RPC: fn_estadisticas_trabajadores_completo
-- Descripción: Estadísticas incluyendo almaceneros (nueva función)
-- =====================================================

CREATE OR REPLACE FUNCTION public.fn_estadisticas_trabajadores_completo(
    p_id_tienda INTEGER
)
RETURNS JSON
LANGUAGE plpgsql
    ...
    SELECT json_build_object(
        ...
        'data', json_build_object(
            ...
            'con_usuario', (
                SELECT COUNT(*) 
                FROM public.app_dat_trabajadores 
                WHERE id_tienda = p_id_tienda AND estado = 1 AND usuario_uuid IS NOT NULL
            ),
            'sin_usuario', (
                SELECT COUNT(*) 
                FROM public.app_dat_trabajadores 
                WHERE id_tienda = p_id_tienda AND estado = 1 AND usuario_uuid IS NULL
            )
        )
    ) INTO v_result;
...
COMMENT ON FUNCTION fn_crear_trabajador_almacenero IS 'Crea trabajador con rol almacenero (función nueva)';
COMMENT ON FUNCTION fn_asignar_rol_almacenero IS 'Asigna rol almacenero a trabajador existente (función nueva)';
COMMENT ON FUNCTION fn_cambiar_almacen_almacenero IS 'Cambia almacén de almacenero (función nueva)';
COMMENT ON FUNCTION fn_listar_trabajadores_completo IS 'Lista trabajadores con todos los roles incluyendo almacenero (función nueva)';
COMMENT ON FUNCTION fn_estadisticas_trabajadores_completo IS 'Estadísticas de trabajadores incluyendo almaceneros (función nueva)';
