DECLARE
  ...
  v_moneda_factura TEXT; -- Variable para manejar la moneda
BEGIN
  ...
  -- Validar y asignar moneda (USD por defecto)
  v_moneda_factura := COALESCE(NULLIF(TRIM(p_moneda_factura), ''), 'USD');
  
  -- Validar que la moneda sea un valor permitido (opcional)
  IF v_moneda_factura NOT IN ('USD', 'EUR', 'CUP') THEN -- Ajusta según tus necesidades
    RAISE EXCEPTION 'Moneda no válida: %', v_moneda_factura;
  END IF;

 -- Validar que el motivo exista y tenga un tipo de operación asociado
  SELECT 1 INTO v_id_tipo_operacion
  ...
  INSERT INTO app_dat_operacion_recepcion (
    ...
    p_motivo,
    NOW(),
    v_moneda_factura  -- Usar la variable validada
  );
  ...
  -- Construir respuesta incluyendo la moneda utilizada
  v_result := jsonb_build_object(
    ...
    'moneda_utilizada', v_moneda_factura,  -- Informar qué moneda se usó
