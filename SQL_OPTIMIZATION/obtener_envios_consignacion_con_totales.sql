...
BEGIN
  ...
  SELECT 
    ...
    COUNT(DISTINCT cep.id)::BIGINT as cantidad_productos,
    COALESCE(SUM(cep.cantidad_propuesta), 0)::NUMERIC as cantidad_total_unidades,
    COALESCE(SUM(cep.cantidad_propuesta * cep.precio_costo_usd), 0)::NUMERIC as valor_total_costo,
    COALESCE(SUM(CASE WHEN cep.precio_venta_cup IS NOT NULL THEN cep.cantidad_propuesta * cep.precio_venta_cup ELSE 0 END), 0)::NUMERIC as valor_total_venta,
    ce.fecha_propuesta,
