...
BEGIN
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE tce.id_tienda = p_id_tienda
      AND tce.activo = TRUE
      AND tce.id_moneda_origen = 2   -- USD (ajusta si tu id es otro)
      AND tce.id_moneda_destino = 1  -- CUP (ajusta si tu id es otro)
  ...
  LEFT JOIN LATERAL (
    ...
    SELECT pic.id_almacen::BIGINT, pic.almacen::TEXT, pic.id_ubicacion::BIGINT, pic.ubicacion::TEXT, pic.id::BIGINT as id_producto,
    ...
    LEFT JOIN stock_reservado sr ON (pic.id = sr.id_producto AND COALESCE(pic.inv_id_variante, 0) = sr.id_variante
        AND COALESCE(pic.inv_id_opcion_variante, 0) = sr.id_opcion_variante AND pic.id_ubicacion = sr.id_ubicacion)
    LEFT JOIN entradas_periodo ent ON (pic.id = ent.id_producto AND COALESCE(pic.inv_id_variante, 0) = ent.id_variante
        AND COALESCE(pic.inv_id_opcion_variante, 0) = ent.id_opcion_variante AND COALESCE(pic.inv_id_presentacion, 0) = ent.id_presentacion
        AND pic.id_ubicacion = ent.id_ubicacion)
    LEFT JOIN extracciones_periodo ext ON (pic.id = ext.id_producto AND COALESCE(pic.inv_id_variante, 0) = ext.id_variante
        AND COALESCE(pic.inv_id_opcion_variante, 0) = ext.id_opcion_variante AND COALESCE(pic.inv_id_presentacion, 0) = ext.id_presentacion
        AND pic.id_ubicacion = ext.id_ubicacion)
    LEFT JOIN ventas_periodo vent ON (pic.id = vent.id_producto AND COALESCE(pic.inv_id_variante, 0) = vent.id_variante
        AND COALESCE(pic.inv_id_opcion_variante, 0) = vent.id_opcion_variante AND COALESCE(pic.inv_id_presentacion, 0) = vent.id_presentacion
        AND pic.id_ubicacion = vent.id_ubicacion)
    LEFT JOIN costo_promedio_productos cp ON (pic.id = cp.id_producto AND COALESCE(pic.inv_id_variante, 0) = cp.id_variante
        AND COALESCE(pic.inv_id_opcion_variante, 0) = cp.id_opcion_variante AND COALESCE(pic.inv_id_presentacion, 0) = cp.id_presentacion)
