-- ============================================================================
-- FUNCIÓN: obtener_reporte_inventario_completo5_completadas_en_periodo
-- Variante de obtener_reporte_inventario_completo5.
-- ============================================================================
-- Diferencias respecto a obtener_reporte_inventario_completo5:
--   Sólo cuenta los movimientos cuya operación alcanzó el estado COMPLETADA (2)
--   DENTRO del período filtrado. Para ello se toma el último registro de
--   app_dat_estado_operacion de cada operación y se exige:
--     - estado = 2
--     - created_at dentro de [v_fecha_desde_ts, v_fecha_hasta_ts]
-- ============================================================================

DROP FUNCTION IF EXISTS public.obtener_reporte_inventario_completo5_completadas_en_periodo(
    BIGINT, TEXT, TEXT, BIGINT, BOOLEAN
);

CREATE OR REPLACE FUNCTION public.obtener_reporte_inventario_completo5_completadas_en_periodo(
    p_id_tienda   BIGINT,
...
BEGIN
    ...
    -- Operaciones asociadas a los movimientos, filtrando solo las que
    -- alcanzaron estado COMPLETADA (2) dentro del período seleccionado
    operaciones_movimientos AS (
        ...
        WHERE ueo.estado = 2
          AND (v_fecha_desde_ts IS NULL OR ueo.created_at >= v_fecha_desde_ts)
          AND (v_fecha_hasta_ts IS NULL OR ueo.created_at <= v_fecha_hasta_ts)
    ),
...
-- Grants de acceso
GRANT EXECUTE ON FUNCTION public.obtener_reporte_inventario_completo5_completadas_en_periodo(BIGINT, TEXT, TEXT, BIGINT, BOOLEAN)
...
-- FROM obtener_reporte_inventario_completo5_completadas_en_periodo(1)
-- LIMIT 10;
-- ============================================================================
