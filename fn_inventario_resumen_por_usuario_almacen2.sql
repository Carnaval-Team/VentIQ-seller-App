CREATE OR REPLACE FUNCTION public.fn_inventario_resumen_por_usuario_almacen2(
    p_id_tienda BIGINT,
    p_id_almacen BIGINT DEFAULT NULL,
    p_busqueda TEXT DEFAULT NULL,
    p_mostrar_sin_stock BOOLEAN DEFAULT TRUE,
    p_filtro_stock VARCHAR DEFAULT 'Todos',
...
RETURNS TABLE (
    ...
    prod_nombre_comercial VARCHAR,
    prod_denominacion_corta VARCHAR,
    prod_descripcion_corta VARCHAR,
    prod_um VARCHAR,
    prod_es_refrigerado BOOLEAN,
    prod_es_fragil BOOLEAN,
    prod_es_peligroso BOOLEAN,
    prod_es_vendible BOOLEAN,
    prod_es_comprable BOOLEAN,
    prod_es_inventariable BOOLEAN,
    prod_es_por_lotes BOOLEAN,
    prod_dias_alert_caducidad NUMERIC,
    prod_codigo_barras VARCHAR,
    prod_imagen TEXT,
    prod_es_elaborado BOOLEAN,
    prod_es_servicio BOOLEAN,
    variante_id BIGINT,
...
BEGIN
    ...
    resumen_filtrado AS (
        ...
        AND (
            -- Filtro de stock en el servidor
            CASE 
                WHEN p_filtro_stock = 'Sin Stock' THEN r.cant_almacen_total <= 0
                WHEN p_filtro_stock = 'Stock Bajo' THEN r.cant_almacen_total > 0 AND r.cant_almacen_total <= 10
                WHEN p_filtro_stock = 'Stock OK' THEN r.cant_almacen_total > 10
                ELSE TRUE -- 'Todos' o cualquier otro valor
            END
        )
    )
    ...
    SELECT 
        ...
        p.descripcion::VARCHAR AS prod_descripcion,
        p.nombre_comercial::VARCHAR AS prod_nombre_comercial,
        p.denominacion_corta::VARCHAR AS prod_denominacion_corta,
        p.descripcion_corta::VARCHAR AS prod_descripcion_corta,
        p.um::VARCHAR AS prod_um,
        p.es_refrigerado::BOOLEAN AS prod_es_refrigerado,
        p.es_fragil::BOOLEAN AS prod_es_fragil,
        p.es_peligroso::BOOLEAN AS prod_es_peligroso,
        p.es_vendible::BOOLEAN AS prod_es_vendible,
        p.es_comprable::BOOLEAN AS prod_es_comprable,
        p.es_inventariable::BOOLEAN AS prod_es_inventariable,
        p.es_por_lotes::BOOLEAN AS prod_es_por_lotes,
        p.dias_alert_caducidad::NUMERIC AS prod_dias_alert_caducidad,
        p.codigo_barras::VARCHAR AS prod_codigo_barras,
        p.imagen::TEXT AS prod_imagen,
        p.es_elaborado::BOOLEAN AS prod_es_elaborado,
        p.es_servicio::BOOLEAN AS prod_es_servicio,
