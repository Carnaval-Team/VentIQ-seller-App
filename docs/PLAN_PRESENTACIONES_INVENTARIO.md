...
- [x] **Transferencia admin**: ya está migrada, el pendiente apuntaba a **dos sitios muertos**. La pantalla real (`inventory_transfer_screen.dart`) **no usa diálogo de cantidad**: usa una lista con un campo por fila, y las filas ya son **una por presentación** — el `variant_key` incluye `id_presentacion` (L178), el dedupe agrupa por `id_producto + id_presentacion` (L187), la fila muestra `presentacion_nombre` (L1039) y el payload manda `id_presentacion` (L241). Transferir «2 Cajas + 3 Bolsas» se hace **en un solo pase**: son dos filas. Ya tiene el trabajo de Fase 2: el validador **no rechaza** pedir más de lo suelto (comentario «FASE 2» en L1089) y muestra un icono `auto_awesome` con el tooltip del rebalanceo.
      **Código muerto detectado — DECISIÓN: se queda, no se toca.** `widgets/transfer_product_quantity_dialog.dart` (archivo entero, **0 usos en el repo**) y la clase privada `_ProductQuantityDialog` de `inventory_transfer_screen.dart` L1331 (**390 líneas, nunca instanciada**; `dart analyze` avisa «A value for optional parameter 'sourceLayoutId' isn't ever given»). Son de cuando la pantalla trabajaba con diálogos. **Ninguno aplana, así que no hay bug** y queda fuera del alcance de este plan: no se borran. Se documentan solo para que nadie los tome por el camino activo de la transferencia — el real es la lista de `inventory_transfer_screen.dart`, que ya está migrada.
...
- [x] `fn_stock_producto_almacen` — **`27_stock_almacen_en_base.sql`, aplicado**. Devolvía `SUM(cantidad_final)` sin factor: con **533 Cajas de 24 decía 533**, no 12.792. Sus consumidores lo comparan contra `cantidad_total_necesaria` de las recetas, que viene en unidades base, así que **subestimaba el stock por el factor** y podía rechazar una producción que sí alcanzaba (`INSUFFICIENT_STOCK_INGREDIENT`).
      **`fn_stock_producto_almacen_detalle` NO se tocó, a propósito**: `fn_descontar_ingredientes_elaborado` lo usa para **escribir** el ledger (`LEAST(cantidad_final, pendiente)` y luego `cantidad_final - a_descontar` en el INSERT), así que su cantidad tiene que seguir siendo la física de esa presentación. Aplicarle el factor guardaría saldos falsos, que es peor que el bug original.
      Compatibilidad: de los 13 consumidores, la mayoría hace **test de signo** (`> 0`, `<= 0`), que no cambia al multiplicar por un factor positivo; los que comparan contra cantidades de receta pasan de estar mal a estar bien. Medido: **6.659 de 6.661 combos producto+almacén devuelven exactamente lo mismo**; los 2 que cambian son 4545 (533 → 12.792) y 4558 (15 → 360), los dos en la tienda 25, que tiene **cocina y modo restaurante apagados** y donde **ninguno de los dos es ingrediente de ninguna receta**. Smoke en la tienda 223 (cocina activa) con 3 elaborados: `fn_validar_ingredientes_elaborado` y `fn_disponibilidad_plato` siguen ejecutando.
      **Trampa del censo:** filtrar por «productos con más de una presentación con saldo» da los combos **equivocados** (3046 y 9753, los de prueba). El bug se dispara con **cualquier** presentación no-base de factor ≠ 1, aunque sea la única con stock — el censo correcto filtra por `factor_rel <> 1`.
      **Deuda anotada:** `fn_descontar_ingredientes_elaborado` L33 compara una cantidad **en presentación** contra un pendiente **en base**. No se arregla aquí porque la solución correcta es que esa función pase por `fn_descontar_con_rebalanceo`, no multiplicar a mano. Hoy no afecta a nadie: los 2 productos con factor ≠ 1 no son ingrediente de ninguna receta.
...
- [x] ✅ **`fn_stock_producto_almacen` ya devuelve unidades base** — `27_stock_almacen_en_base.sql`, aplicado. Ver el detalle en Fase 3 § Stock. **Corrección del análisis que había aquí:** el censo «1 sola combinación con más de una presentación con stock» era el **equivocado** — el bug se dispara con cualquier presentación no-base de factor ≠ 1 aunque sea la única con saldo, y así el censo da **2 combinaciones reales** (4545 «FREEGELLS» 533 Cajas ×24 → 12.792 y 4558 «Salchicha» 15 → 360, ambas en la tienda 25). El detalle **no** se tocó: lo usa `fn_descontar_ingredientes_elaborado` para escribir el ledger.
...
- [x] **`fn_inventario_detallado_optimizado`** auditada y corregida (`presentaciones_inventario/26_inventario_detallado_fanout.sql`, **aplicado**). Resultado: **no aplana** — los 4 CTE de movimientos (entradas, extracciones, ventas, costo) ya agrupaban por `COALESCE(id_presentacion, 0)` y sus JOIN casan por esa clave. Pero tenía **dos bugs**, los dos medidos:
      **(a) el reservado se contaba dos veces.** `stock_reservado` era el único CTE sin la presentación en el `GROUP BY` ni en el JOIN, así que con dos presentaciones con saldo en la misma ubicación la fila de reservado casaba con ambas: **15.460,30 real → 30.920,60 reportado** en 3 combinaciones (productos 217, 3046, 9753). No era «hoy inocuo»: ya estaba mal.
      **(b) fan-out del precio de venta, y esto no es de presentaciones.** El `LEFT JOIN app_dat_precio_venta` sin desempatar casaba con **todas** las filas activas y el `SELECT DISTINCT` no las colapsaba porque el precio difiere. El producto 217 tiene **7 filas con `fecha_hasta IS NULL`** (196 / 104,18 / 99,22 / 94,50 / 105 / 100) → la función devolvía **18 filas donde hay 3**. Alcance del problema de datos: **2.198 productos con más de una fila de precio activa, 1.040 con precios distintos simultáneos, hasta 24 filas en un producto**. Resuelto con `LEFT JOIN LATERAL … ORDER BY fecha_desde DESC, id DESC LIMIT 1`.
      **Por qué fue seguro:** la función **no la llama nadie** — 0 consumidores en `pg_proc.prosrc` y 0 en Dart (buscado en los dos proyectos). Se arregló ahora que está medido para que no sea una trampa el día que se conecte a una pantalla.
...
- [x] ~~`transfer_product_quantity_dialog.dart`~~ — **falsa alarma**: es código muerto (0 usos). La transferencia admin real ya es una fila por presentación con su propio campo, así que el mixto se puede en un solo pase. Se deja sin tocar por decisión explícita
