CREATE OR REPLACE FUNCTION fn_reporte_ventas_gananciasv4(
    p_id_tienda INTEGER,
...
BEGIN
    ...
    SELECT 
        ...
        -- ✅ CAMBIO: Usar precio_promedio de app_dat_producto_presentacion en lugar de app_dat_recepcion_productos
        ...
        -- ✅ CAMBIO: Usar precio_promedio de presentación para cálculo de costo
    ...
    LEFT JOIN (
        -- ✅ NUEVO: Obtener precio_promedio de app_dat_producto_presentacion (costo real de la presentación)
        SELECT 
            pp.id_producto,
...
-- ============================================================================
-- CAMBIOS REALIZADOS EN fn_reporte_ventas_gananciasv4:
-- ============================================================================
-- 
-- 1. ✅ AGREGADO: JOIN con app_dat_producto_presentacion (pp)
--    - Obtiene precio_promedio de la presentación específica del producto
--    - Filtra solo presentaciones con precio_promedio > 0
--    - Usa id_presentacion de app_dat_extraccion_productos para el JOIN
--
-- 2. ✅ MODIFICADO: Cálculo de precio_costo
--    - ANTES: COALESCE(rp.precio_costo, 0)
--    - AHORA: COALESCE(pp.precio_promedio, rp.precio_costo, 0)
--    - Prioridad: precio_promedio de presentación > costo de recepción > 0
--
-- 3. ✅ MODIFICADO: Cálculo de precio_costo_cup
--    - Usa el nuevo precio_costo que incluye precio_promedio de presentación
--
-- 4. ✅ MODIFICADO: Cálculo de costo_total_vendido
--    - Usa el nuevo precio_costo que incluye precio_promedio de presentación
--
-- 5. ✅ MODIFICADO: Cálculo de ganancia_unitaria
--    - Usa el nuevo precio_costo que incluye precio_promedio de presentación
--
-- 6. ✅ MODIFICADO: Cálculo de ganancia_total
--    - Usa el nuevo precio_costo que incluye precio_promedio de presentación
--
-- 7. ✅ AGREGADO: id_presentacion en SELECT del subquery ventas
--    - Necesario para hacer JOIN con app_dat_producto_presentacion
--
-- ============================================================================
-- BENEFICIOS:
-- ============================================================================
-- 
-- ✅ Costo real por presentación: Usa precio_promedio de cada presentación
-- ✅ Más preciso: Refleja el costo real de compra de esa presentación específica
-- ✅ Fallback automático: Si no hay precio_promedio, usa costo de recepción
-- ✅ Cálculos correctos: Ganancias basadas en costo real de la presentación
-- ✅ Compatible: Mantiene la misma estructura de salida
--
-- ============================================================================
