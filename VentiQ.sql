...
create table public.app_dat_cambio_precio (
  ...
  motivo text null,
  fecha_cambio timestamp with time zone not null default now(),
  monto_descontado numeric null,
  constraint app_dat_cambio_precio_pkey primary key (id),
  constraint fk_producto foreign KEY (id_producto) references app_dat_producto (id),
  constraint fk_tpv foreign KEY (id_tpv) references app_dat_tpv (id),
  constraint fk_usuario foreign KEY (id_usuario) references auth.users (id),
  constraint fk_variante foreign KEY (id_variante) references app_dat_variantes (id)
