...
| 28 | `28_operaciones_con_presentacion.sql` | 3 | si (**el `||` de jsonb solo AGREGA claves**; las 6 que lee la app vieja quedan intactas y el `RETURNS TABLE` no cambia) | La lista de Operaciones nunca mostro la presentacion: `fn_listar_operaciones_inventario_new` **no mencionaba `id_presentacion` ni una vez** en sus 32.828 caracteres. Se concatena `fn_presentacion_item_json` en los **7** bloques de items (4 `ep` + 2 `rp` + 1 `cp`). El de ajustes (`ai`) se salta: `app_dat_ajuste_inventario` **no tiene** esa columna. |
| 29 | `29_reporte_ventas_v5.sql` | 3 | **total: funcion NUEVA, la v4 no se toca** (decision del usuario: `_vx` en vez de modificar) | `fn_reporte_ventas_con_proveedor5`. La v4 agrupa por presentacion Y por precio pero **no expone ninguno**: devolvia **1.377 filas para 671 productos** en la tienda 45 (**420 productos con varias filas indistinguibles en la UI**, todas por precio distinto). La v5 **envuelve** a la v4 (hereda su logica de costos historicos, 17.314 caracteres que no conviene reimplementar), compacta a una fila por producto con precio/costo **ponderados**, y agrega `cantidades_por_presentacion`, `equiv_unidades_base`, `cantidad_formateada`, `n_presentaciones` y `n_precios_distintos`. |
| 30 | `30_cierre_vendedor_v2.sql` | 6 | **total: dos funciones NUEVAS, las originales no se tocan** | `fn_resumen_diario_cierre_v2` y `fn_productos_vendidos_por_turno_v2`. **Tres bugs, solo uno de presentaciones.** (a) `productos_vendidos` era `integer` y `SUM(...)::INTEGER` **REDONDEA**: tres lineas de 0,5 kg daban 2 (133 lineas fraccionadas en 30 dias, 2 tiendas). (b) el `LEFT JOIN app_dat_pago_venta` de la del detalle **multiplicaba cada linea por cada pago** de la operacion, inflando cantidad E importe (93 ops en 30 dias, hasta x3). (c) el JOIN a `app_nom_presentacion` casaba `pp.id` contra `np.id` (trampa `pp.id <> pp.id_presentacion`) y etiquetaba **TODO como 'Unidad'**. Nuevas columnas: `productos_vendidos_base`, `productos_vendidos_desglose`, `factor_rel`, `cantidad_base`, `cantidad_formateada`. |
| 31 | `31_inventario_resumen_stock_mixto_v3.sql` | visualizacion stock | si (funcion NUEVA; v2 intacta) | `fn_inventario_resumen_por_usuario_almacen3`: wrapper de la v2 viva con sus 14 columnas en el mismo orden y cuatro `stock_*` al final. Desglose por producto+variante+opcion, sin descripcion inventada. `SECURITY INVOKER`, `search_path=''`, guarda de tienda y ACL solo `authenticated`. |
| 32 | `32_stock_mixto_producto_por_ubicacion.sql` | visualizacion stock | si (funcion NUEVA) | Una fila por ubicacion con almacen, desglose fisico, textos y equivalente base. Agrega variantes/opciones solo dentro de la misma presentacion; nunca suma Cajas con Bultos. Misma seguridad endurecida del 31. |
| 34 | `34_tests_visualizacion_stock.sql` | visualizacion stock | no (`BEGIN`/`ROLLBACK`) | Tests transaccionales: `49 Cajas + 2.8 Bultos = 1190 base`, nunca `51.8`; cero omitido; multiubicacion; v3 14+4 columnas; paginacion; ACL/search_path/invoker y acceso cruzado. Requiere sustituir un UUID con acceso a tienda 223. |
| 43 | `43_catalogo_stock_mixto_v3.sql` | catalogo vendedor | **total: 3 funciones NUEVAS** (`fn_catalogo_stock_meta`, `get_productos_by_categoria_tpv_search_meta_v3`, `get_productos_by_categoria_tpv_v2`); las vivas no se tocan | El catalogo sumaba `SUM(ip.cantidad_final)` sin ponderar por presentacion: con Cajon x30 (13) + Caja x24 (48) + Blister x6 (73) + Unidad x1 (19) el vendedor leia **153**. La v3 conserva las 18 columnas y añade al `metadata` deltas: `stock_desglose`, `stock_texto` («13 Cajones + 48 Cajas + 73 Blisteres + 19 Unidades»), `stock_texto_corto`, `stock_equivalente_base` (**1999**), `stock_total_fisico`, `stock_filas`, `stock_n_presentaciones`, `stock_mixto`, `stock_con_variantes`. La v2 arregla ademas un bug real: el `LEFT JOIN app_dat_precio_venta` sin desempatar devolvia el producto N veces (**3.496 productos con >1 precio activo, hasta 25 filas**). |
| 44 | `44_tests_catalogo_stock_mixto_v3.sql` | catalogo vendedor | no (solo lectura; los de JWT con `BEGIN`/`ROLLBACK`) | Bloques A-F: el caso 153/1999, no-regresion viva vs v3 (8 filas = 8 filas, **0 diferencias** en stock/tiene_stock/precio/metadata), contrato de claves (4 vivas + 9 nuevas), casos borde del helper, fan-out de precio de la v2, ACL por `aclexplode`, aislamiento entre tiendas y el pitfall del `search_path`. |

Los metadatos del contrato remoto usado por el 31 y el baseline de Advisors estan en `contracts/README.md`. Los archivos 31, 32 y 34 estan preparados localmente y **no aplicados remotamente**.
...
- [x] **30 APLICADO** y verificado: originales **intactas** (`fn_resumen_diario_cierre` 17 columnas / 6.345 caracteres, `fn_productos_vendidos_por_turno` 10 columnas / 3.274); v2 con 19 y 13 columnas, `SECURITY DEFINER` + `search_path=public` + `check_user_has_access_to_any_tienda` (probado: sin JWT aborta con «Acceso denegado»). **Contraste contra el ledger** en el turno 2948: producto 5683 ledger 1,0 / 24.480,00 → original **2,0 / 48.960,00** → v2 **1,0 / 24.480,00**; producto 5709 igual; total del turno 44.880,00 real vs **89.760,00** que reportaba la original. Turno 1225 (248 lineas fraccionadas): `productos_vendidos` 55512 → **55511.89**, con ventas y operaciones **identicas**. Archivo local == produccion (md5 sin comentarios `2fc12947` / 4.369 y `9408d3b1` / 2.878). **Trampa:** el archivo y produccion divergieron 22 caracteres porque al aplicar quite un `ORDER BY` dentro de `string_agg(DISTINCT ...)` (Postgres exige que sea por la misma expresion) y no lo reflejé en el archivo; lo cazo la comprobacion de md5.
- [ ] **31, 32 y 34 SOLO LOCALES, NO APLICADOS**: SQL y tests de visualizacion de stock mixto. El 31 conserva exactamente el contrato remoto v2 de 14 columnas y añade 4 `stock_*`; el 32 entrega el stock por ubicacion; el 34 prueba calculo, paginacion y seguridad. Pendiente aplicar primero 31/32 y ejecutar 34 con un UUID de tienda 223.
- [x] **29 APLICADO** y verificado: v4 **intacta** (16 columnas, 17.314 caracteres, 1 sobrecarga); v5 con 21 columnas, `SECURITY DEFINER` + `search_path=public` + `check_user_has_access_to_tienda` (probado: sin JWT aborta con «Acceso denegado»). Cuadre tienda 45: **1.377 → 671 filas** con ingresos 160.672.118,68, costo 143.436.891,59 y cantidad 64.744,59 **identicos**; 0 productos repetidos. Con 30 dias y con `completado`: 331 → 318, ingresos iguales. Tiempo ~129 ms (la v4 anidada ~78 ms). Archivo local == produccion (md5 sin comentarios `365cf09c`, 4.760 caracteres). **Trampa que cometi y corregi en caliente:** agrupar el desglose por `id_presentacion` a secas hace que las lineas historicas sin presentacion salgan como una **presentacion fantasma** («149 Unidades + 131», n_presentaciones 2); el nulo es la **base** y hay que resolverlo antes de agrupar (ahora «280 Unidades», n_presentaciones 1). Es el mismo error del `26`.
- [x] **28 APLICADO** y verificado: 7 inserciones (`ep` 4 / `rp` 2 / `cp` 1), claves viejas intactas (`sku_producto` 8, `producto_nombre` 8, `importe` 4), 1 sobrecarga, sigue `SECURITY DEFINER`. Sobre 305 items de la tienda 45: **297 con presentacion**, 0 perdieron `id_producto`/`producto_nombre`/`sku_producto`. Los 8 sin `cantidad` son de **Ajuste Negativo**, que nunca la tuvo (usa `cantidad_anterior`/`cantidad_nueva`). Plural verificado: «1 Bulto» / «4 Bultos» / «533 Cajas» (equiv 12.792).
...
- [x] **43 APLICADO** y verificado (2026-09-14): **3 funciones nuevas**, las vivas intactas (`search_meta` 10.058 caracteres y `tpv` 5.110, `proconfig` NULL: nadie las reemplazo). No-regresion en la tienda 223: **8 filas = 8 filas**, 0 diferencias en `stock_disponible`, `tiene_stock`, `precio_venta`, `denominacion` y las 4 claves de metadata de antes; contrato 4 claves vivas -> 13 en la v3. Caso real producto 11007: fisico **153**, equivalente **1999**, texto «13 Cajones + 48 Cajas + 73 Blisteres + 19 Unidades»; producto de prueba «prod azucar multiipresentacion»: 51.8 -> «49 Cajas + 2.8 Bultos» = 1190 base. **2 productos mixtos en esa tienda.** ACL solo `authenticated` (verificado con `aclexplode`), `search_path=''` con `SECURITY DEFINER`, y aislamiento entre tiendas probado (pedir la tienda 11 con JWT de la 223 aborta con P0001). Tests en el `44` (bloques A-F).
- [x] **44 ESCRITO** y ejecutado bloque a bloque contra produccion. Todo de solo lectura; los que necesitan JWT van con `BEGIN`/`ROLLBACK` + `request.jwt.claims`.
- [x] **44 APLICADO** (solo lectura, sin cambios de datos).
- [x] **Bug propio cazado al ensayar el 43**: `SET search_path = ''` + `check_user_has_access_to_tienda` (que resuelve tablas SIN calificar) = **42P01 «relation "app_dat_vendedor" does not exist»**. Se corrigio con `PERFORM set_config('search_path', 'public, pg_catalog', false);` **antes** de la guarda, en las 3 funciones. Es el mismo patron que ya documentaba el archivo 32. Sin esto, la primera version de la v3 fallaba en el 100 % de las llamadas.
